...
- **App metadata (crawl_state) is stored in the plain DuckDB catalog** (`catalog/vworld_catalog.db`), not in DuckLake. This is a deliberate separation: DuckLake is for geospatial data tables (roads, buildings, rejects); the DuckDB catalog is for app-internal state (URLs, ETags, download timestamps). This avoids polluting the DuckLake console with internal tables and sidesteps DuckLake's lack of PRIMARY KEY/INSERT OR REPLACE support.
